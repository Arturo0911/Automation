...
class SocketConnection:
    ...
    def socket_accept_connection(self):
        ...
        try:
            ...
            print("[*] Session opened at %s %s" % (self.address[0], self.address[1]))
            print("\n")
...
# initializer this function to get the global variables in whole the code.
global_variables()
...
def showing():
    ...
    -r --reverse            if putting a host target and specific port to try to get reverse shell""")


def show_port_opened(name_host, name_port):
    """The purpose is get and discover which port is opened"""

    host_name = check_ip(name_host)
    ...
    try:
        ...
        try:
            ...
            port_name = socket.getnameinfo((str(name_host), name_port), socket.NI_NUMERICHOST)
